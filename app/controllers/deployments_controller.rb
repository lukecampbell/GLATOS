class DeploymentsController < ApplicationController
  ...
  def index
    ...
    respond_to do |format|
      ...
      format.geo {
        if params[:study_id]
          deps = Deployment.active_study.includes(:study, :retrieval, :otn_array).where(["study_id = ?",study])
        else
          deps = Deployment.active_study.includes(:study, :retrieval, :otn_array).order('study_id ASC')
        end
        render :json =>
          deps.as_json({
            :only => [nil],
